...
|------------|----------|----------------|----------------|-----------------|--------|
| 7-A | Feasibility formula validity | Dimensionally broken (use min(B_01,B_12) >= AI×Fcompute) | Dimensionally valid (use 1/B_01+1/B_12 <= 1/Fcompute) | Both formulas implemented and run on startup. At 8.48 GB/s (MIN_NVME_BANDWIDTH_GBS), both formulas return FAIL — they agree. At 100 GB/s both return PASS — they agree. In the narrow band 8.49–9.27 GB/s the corrected formula passes while the original still fails, confirming Source A is strictly more lenient. Verified by test_dispute_7a_bandwidth_boundary (exact boundary: payload/compute = 4132/487 ≈ 8.4846 GB/s). | Source A (safer) |
